...
public class Vessel : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // get references
        PlayerShip = GameObject.Find("Player").transform.GetChild(0).gameObject;
    ...
    private void CheckForObstructions()
    {
        ...
        // if there is a way to the target, abandon the waypoint
        if (Waypoint != null)
        {
    ...
    public void Define(VesselPayload payload)
    {
        var renderer = this.GetComponent<MeshRenderer>();
        transform.position = new Vector3(payload.x, payload.y, payload.z);
    }

    public static Vessel Instantiate(VesselPayload payload)
    {
        var prefab = Resources.Load($"Prefabs/{payload.designation}");
        GameObject obj = (GameObject)GameObject.Instantiate(prefab);
        obj.name = payload.id;
        var vessel = obj.GetComponent<Vessel>();
        vessel.Define(payload);
        return vessel;
    }
